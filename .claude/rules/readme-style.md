...
# MkPFS - README & Markdown Style Guide

Purpose: capture the visual and prose style we use across README.md, docs pages, and short wiki-style markdown files so contributors produce consistent, high-quality docs.
...
- Iconography: use UTF-8 emoji for section headings and quick-links. Keep icons consistent and semantically appropriate. Examples:
  ...
  - 🔗 Related projects
  - 💖 Sponsor
- Badges: prefer shields.io for badges. Style: `style=for-the-badge` for the top cluster. Include: status, PyPI (version), license, Python version, docs link. Use flat-square for a second-row group when needed.
...
- Center the hero block. Include in order:
  ...
  4. Top badge cluster (status, PyPI, license, python, docs).
  5. Quick-links row (emoji-prefixed links separated by middot ·). Order:
     - 📚 Documentation · 📦 Install · ⌨️ Commands · 🖥️ GUI · 💙 Thanks · 🔗 Related projects
     - then a blank line
...
- Typical README section order:
  ...
  9. Contributing (short callout linking to docs/contributing.md)
...
- Prefer linking to the published MkDocs site for user-facing guides (use full URLs to the subpages):
  - https://renangbarreto.github.io/MkPFS/getting-started/
  - https://renangbarreto.github.io/MkPFS/commands/
  - https://renangbarreto.github.io/MkPFS/knowledge/
- Keep in-repo docs pages under `docs/` for editing, but point public links to the published site.
...
- Include a short contributing pointer linking to `CONTRIBUTING.md` or `docs/contributing.md`.
...
- Before merging README or major docs changes run:
  ...
  - `uv run --frozen pytest`
  - `uv run mkdocs build --strict`
- The repository uses the `mkdocs-material` theme; changes to nav and knowledge-base pages can produce informational warnings — fix obvious broken relative links when found.
