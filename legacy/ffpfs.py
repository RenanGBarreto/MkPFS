...
def validate_d32_ranges(inodes: list[Inode], final_ndblock: int) -> None:
    ...
    if final_ndblock > INT32_MAX:
        raise BuildError(f"Image requires block index {final_ndblock}, exceeds D32 pointer limit {INT32_MAX}")

    for ino in inodes:
        if not (0 <= ino.number <= UINT32_MAX):
...
def signed_inode_capacity_bytes(block_size: int) -> int:
    sigs_per_block = block_size // SIG_ENTRY_SIZE
    if sigs_per_block <= 0:
        return 0
...
def compose_pfs_mode(inode_bits: int, case_insensitive: bool) -> int:
    ...
    mode = 0
    if inode_bits == 64:
        mode |= PFS_MODE_64BIT_INODES
    if case_insensitive:
        mode |= PFS_MODE_CASE_INSENSITIVE
    return mode
...
def build_inode_block_sig_s64(inode_block_count: int, block_size: int, now: int, signed: bool = False) -> bytes:
    ...
    struct.pack_into("<I", sig, 0x04, 0 if signed else INODE_FLAG_READONLY)  # flags
...
class Inode:
    ...
    size_compressed: int
    blocks: int
    db: list[int] = field(default_factory=lambda: [0] * MAX_DIRECT_BLOCKS)
    ib: list[int] = field(default_factory=lambda: [0] * MAX_INDIRECT_BLOCKS)
    db_sig: list[bytes] = field(default_factory=lambda: [b"\x00" * SIG_SIZE for _ in range(MAX_DIRECT_BLOCKS)])
    ib_sig: list[bytes] = field(default_factory=lambda: [b"\x00" * SIG_SIZE for _ in range(MAX_INDIRECT_BLOCKS)])
    time_sec: int = 0
    ...
    def to_bytes(self) -> bytes:
        ...
        out += struct.pack("<" + "i" * MAX_DIRECT_BLOCKS, *self.db)
        out += struct.pack("<" + "i" * MAX_INDIRECT_BLOCKS, *self.ib)
        if len(out) != INODE_D32_SIZE:
...
def scan_source_tree(root: Path, progress: Progress) -> tuple[dict[str, DirNode], dict[str, FileNode], int]:
    ...
    for i, abs_path in enumerate(abs_files, start=1):
        ...
        if parent == ".":
            parent = ""
        parts = [part for part in Path(rel).parts[:-1]]
        ...
        for h in sorted(by_hash.keys()):
            ...
            for full_path, ino_num, is_dir in entries:
                d = Dirent(
                    ...
                    type_code=DIRENT_TYPE_DIRECTORY if is_dir else DIRENT_TYPE_FILE,
                    name=full_path,
                )
...
def _compute_file_storage_worker(args: tuple[Path, int, bool, int, int]) -> tuple[Path, bytes, int, bool, float, int]:
    """Worker function for parallel compression. Returns (file_path, stored_payload, stored_size, compressed, gain_pct, hypothetical_compressed_size)."""
...
def signed_inode_sig_offset(inode_number: int, ptr_index: int, block_size: int) -> int:
    inodes_per_block = block_size // INODE_S32_SIZE
    ...
    inode_offset = block_size + (inode_table_block * block_size) + (inode_index_in_block * INODE_S32_SIZE)
    return inode_offset + 0x64 + (SIG_ENTRY_SIZE * ptr_index)
...
def make_sig_records_blob(blocks: list[int], block_size: int) -> bytes:
    ...
    offset = 0
    for block in blocks:
        struct.pack_into("<i", blob, offset + SIG_SIZE, block)
        offset += SIG_ENTRY_SIZE
    return bytes(blob)
...
def collect_signed_block_numbers(
    ...
    sigs_per_block = block_size // SIG_ENTRY_SIZE
    blocks: list[int] = []
    direct_count = min(inode.blocks, MAX_DIRECT_BLOCKS)
...
def write_payload_to_blocks(out, payload: bytes, blocks: list[int], block_size: int) -> None:
...
def assign_signed_inode_layout(
    ...
    for i in range(MAX_DIRECT_BLOCKS):
        inode.db[i] = 0
    for i in range(MAX_INDIRECT_BLOCKS):
        inode.ib[i] = 0

    direct_count = min(block_count, MAX_DIRECT_BLOCKS)
    ...
    for _ in range(simple_count):
        ...
        sig_targets.append(
            SignatureTarget(
                child_block, ib0_block * block_size + len(ib0_children[:-1]) * SIG_ENTRY_SIZE, block_size, 0
            )
        )
    ...
    for idx in range(sigs_per_block):
        ...
        sig_targets.append(
            SignatureTarget(child_indirect_block, ib1_parent * block_size + idx * SIG_ENTRY_SIZE, block_size, 1)
        )
        ...
        for rec_idx in range(child_count):
            ...
            sig_targets.append(
                SignatureTarget(
                    data_block, child_indirect_block * block_size + rec_idx * SIG_ENTRY_SIZE, block_size, 0
                )
            )
...
def build_pfs(
    ...
    super_root_inode = Inode(
        ...
        nlink=1,
        flags=INODE_FLAG_INTERNAL
        | (0 if signed else INODE_FLAG_READONLY)
        | (INODE_FLAG_SIGNED_EXTRA if signed else 0),
        size=block_size,
    ...
    uroot_inode = Inode(
        ...
        mode=INODE_MODE_DIR | INODE_RX_ONLY,
        nlink=3,
        flags=(0 if signed else INODE_FLAG_READONLY) | (INODE_FLAG_SIGNED_EXTRA if signed else 0),
        size=block_size,
        size_compressed=block_size,
        blocks=1,
        time_sec=now,
    ...
    for d in non_root_dirs:
        ino = Inode(
            ...
            mode=INODE_MODE_DIR | INODE_RX_ONLY,
            nlink=2,
            flags=INODE_FLAG_READONLY | (INODE_FLAG_SIGNED_EXTRA if signed else 0),
            size=block_size,
            size_compressed=block_size,
            blocks=1,
            time_sec=now,
    ...
    for f in file_nodes_sorted:
        flags = (
            INODE_FLAG_READONLY
            | (INODE_FLAG_COMPRESSED if f.compressed else 0)
            | (INODE_FLAG_SIGNED_EXTRA if signed else 0)
        )
        ...
        file_size_compressed = f.stored_size if f.compressed else f.stored_size
        ino = Inode(
            number=next_inode_number,
            mode=INODE_MODE_FILE | INODE_RX_ONLY,
            nlink=1,
    ...
    for d in dir_nodes_sorted:
        ...
        d.dirents = [
            Dirent(this_ino.number, DIRENT_TYPE_DOT, "."),
            Dirent(parent_ino.number if d.rel_dir != "" else this_ino.number, DIRENT_TYPE_DOTDOT, ".."),
        ]

        for child_rel_dir in d.children_dirs:
            child_dir = dirs[child_rel_dir]
            d.dirents.append(Dirent(child_dir.inode.number, DIRENT_TYPE_DIRECTORY, child_dir.name))
            this_ino.nlink += 1

        for child_rel_file in d.children_files:
            child_file = files[child_rel_file]
            d.dirents.append(Dirent(child_file.inode.number, DIRENT_TYPE_FILE, child_file.name))
    ...
    if has_collision:
        collision_inode = Inode(
            ...
            mode=INODE_MODE_FILE | INODE_RX_ONLY,
            nlink=1,
            flags=INODE_FLAG_INTERNAL | INODE_FLAG_READONLY | (INODE_FLAG_SIGNED_EXTRA if signed else 0),
    ...
    super_root_dirents: list[Dirent] = [Dirent(fpt_inode.number, DIRENT_TYPE_FILE, "flat_path_table")]
    if has_collision and collision_inode is not None:
        super_root_dirents.append(Dirent(collision_inode.number, DIRENT_TYPE_FILE, "collision_resolver"))
    super_root_dirents.append(Dirent(uroot_inode.number, DIRENT_TYPE_DIRECTORY, "uroot"))

    inode_count = len(inodes)
    inode_size = INODE_S32_SIZE if signed else INODE_D32_SIZE
    ...
    if signed:
        ...
        for inode, payload, is_dir in all_nodes_data:
            ...
            if is_dir:
                ...
                if inode.flags & INODE_FLAG_COMPRESSED:
        ...
        signature_targets.append(SignatureTarget(0, HEADER_DIGEST_OFFSET, HEADER_DIGEST_SIZE, 4))
    else:
        ndblock = 1
        ndblock += inode_block_count
        ...
        for i in range(1, MAX_DIRECT_BLOCKS):
            fpt_inode.db[i] = -1
    ...
    try:
        ...
        with tmp_path.open("w+b") as out:
            ...
            struct.pack_into("<q", hdr, 0x08, PFS_MAGIC)
            ...
            if signed:
                ...
                hdr[0x370 : 0x370 + len(ZERO_PFS_SEED)] = ZERO_PFS_SEED
            else:
            ...
            if signed:
                sign_key = pfs_gen_sign_key(ZERO_EKPFS, ZERO_PFS_SEED)
                for level in range(5):
                    for target in (t for t in signature_targets if t.level == level):
                        ...
                        if 0 <= sig_pos_in_block <= len(block_data) - SIG_SIZE:
                            block_data[sig_pos_in_block : sig_pos_in_block + SIG_SIZE] = b"\x00" * SIG_SIZE
...
def validate_image_quick(
    ...
    if version != expected_version or magic != PFS_MAGIC:
    ...
    signed = (expected_mode & PFS_MODE_SIGNED) != 0
    inode_size = INODE_S32_SIZE if signed else INODE_D32_SIZE
    ...
    with image_path.open("rb") as f:
        ...
        for _ in range(dinode_blocks):
            ...
            for idx in range(inodes_per_block):
                ...
                if mode_bits & INODE_MODE_ANY_WRITE:
                    raise BuildError(
                        f"Post-write validation failed: inode {parsed} has write bits set (mode=0x{mode_bits:04X})"
                    )
                if not signed and (flags_bits & INODE_FLAG_READONLY) == 0:
                    raise BuildError(
                        f"Post-write validation failed: inode {parsed} missing readonly flag (flags=0x{flags_bits:08X})"
                    )
                parsed += 1


def prompt_overwrite(output_path: Path) -> bool:
    """Prompt user if output file exists. Returns True if should proceed."""
...
def print_build_parameters(
    ...
    _ver_label = "PS5" if pfs_version == PFS_VERSION_PS5 else "PS4"
    print(f"  Version:           {pfs_version} ({_ver_label})")
    print(f"  Format:            {PFS_MAGIC}")
    print(f"  Block size:        {block_size:,} bytes ({block_size // 1024} KiB)")
    print(f"  Inode width:       {inode_bits}-bit")
    print(
        f"  PFS mode:          0x{mode:04X}  (Bit 0=signed, Bit 1=64-bit inodes, Bit 2=encrypted, Bit 3=case insensitive)"
    )
    print(f"    Signed:          {'yes' if mode & PFS_MODE_SIGNED else 'no'}")
    print(f"    64-bit inodes:   {'yes' if mode & PFS_MODE_64BIT_INODES else 'no'}")
    print(f"    Encrypted:       {'yes' if mode & PFS_MODE_ENCRYPTED else 'no'}")
    print(f"    Case insensitive: {'yes' if mode & PFS_MODE_CASE_INSENSITIVE else 'no'}")
...
def print_summary(stats: BuildStats) -> None:
    ...
    if stats.compression_enabled:
        ...
        print(
            f"    Max theoretical gain:   {stats.max_possible_gain_pct:.2f}%  ({human_readable_size(stats.all_compressed_total_size)} if all files compressed)"
        )
    else:
    ...
    print(
        f"    Wasted space:           {human_readable_size(stats.block_alignment_waste)} ({waste_pct:.2f}% of file data blocks)"
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Minimum gain percent to keep a file compressed (default: 20)",
    )
    parser.add_argument(
        "--block-size",
        default="auto",
        help="PFS block size in bytes, or 'auto' (default: auto=65536)",
    )
    parser.add_argument(
        "--version",
        choices=["PS4", "PS5"],
        default="PS4",
        help="PFS profile version (default: PS4)",
    )
    parser.add_argument(
        "--inode-bits",
        type=int,
    ...
    case_group.add_argument(
        "--case-sensitive",
        action="store_true",
        help="Build a case-sensitive image",
    )
    case_group.add_argument(
        "--case-insensitive",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of CPU cores to use for compression (0 = all available)",
    )
    parser.add_argument(
        "--compression-level",
        type=int,
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Case-insensitive is the default (like real PFS images)
    # Only switch to case-sensitive if --case-sensitive is explicitly passed
    case_insensitive = args.case_insensitive or not args.case_sensitive
    pfs_version = PFS_VERSION_PS5 if args.version == "PS5" else PFS_VERSION_PS4
...
class ParsedInode:
    ...
    def is_dir(self) -> bool:
        return (self.mode & INODE_MODE_DIR) != 0

    @property
    def is_file(self) -> bool:
        return (self.mode & INODE_MODE_FILE) != 0

    @property
    def is_compressed(self) -> bool:
        return (self.flags & INODE_FLAG_COMPRESSED) != 0
...
def _read_exact(fh, offset: int, size: int) -> bytes:
    fh.seek(offset)
    data = fh.read(size)
    if len(data) != size:
...
def parse_image_header(fh) -> ParsedHeader:
...
def parse_image_inode(blob: bytes, number: int, signed: bool) -> ParsedInode:
    expected_size = INODE_S32_SIZE if signed else INODE_D32_SIZE
    ...
    if signed:
        ...
        ib: list[int] = []
        offset = 0x64
        for _ in range(MAX_DIRECT_BLOCKS):
            db_sig.append(blob[offset : offset + SIG_SIZE])
            db.append(struct.unpack_from("<i", blob, offset + SIG_SIZE)[0])
            offset += SIG_ENTRY_SIZE
        for _ in range(MAX_INDIRECT_BLOCKS):
            ib_sig.append(blob[offset : offset + SIG_SIZE])
            ib.append(struct.unpack_from("<i", blob, offset + SIG_SIZE)[0])
            offset += SIG_ENTRY_SIZE
        return ParsedInode(
            number=number,
            mode=mode,
...
def parse_image_inodes(fh, header: ParsedHeader) -> list[ParsedInode]:
    inodes: list[ParsedInode] = []
    signed = (header.mode & PFS_MODE_SIGNED) != 0
    inode_size = INODE_S32_SIZE if signed else INODE_D32_SIZE
...
def parse_sig_record_block(fh, block_num: int, block_size: int) -> list[tuple[bytes, int]]:
    blob = _read_exact(fh, block_num * block_size, block_size)
    records: list[tuple[bytes, int]] = []
    for offset in range(0, block_size, SIG_ENTRY_SIZE):
        if offset + SIG_ENTRY_SIZE > block_size:
            break
        sig = blob[offset : offset + SIG_SIZE]
        block = struct.unpack_from("<i", blob, offset + SIG_SIZE)[0]
...
def block_hmac_without_slot(block_data: bytes, sig_offset_in_block: int, size: int, signed: bool = True) -> bytes:
    ...
    if signed and 0 <= sig_offset_in_block <= len(chunk) - SIG_SIZE:
        chunk[sig_offset_in_block : sig_offset_in_block + SIG_SIZE] = b"\x00" * SIG_SIZE
    return bytes(chunk)
...
def verify_signed_image_signatures(
    ...
    if (header.mode & PFS_MODE_SIGNED) == 0:
        return

    sign_key = pfs_gen_sign_key(ZERO_EKPFS, header.seed)
    ...
    for i in range(header.dinode_block_count):
        ...
        actual = _read_exact(fh, sig_offset, SIG_SIZE)
        if actual != expected:
            errors.append(f"inode block signature mismatch for block {block_num}")

    header_region = bytearray(_read_exact(fh, 0, HEADER_DIGEST_SIZE))
    header_region[HEADER_DIGEST_OFFSET : HEADER_DIGEST_OFFSET + SIG_SIZE] = b"\x00" * SIG_SIZE
    expected_header_sig = hmac_sha256(sign_key, bytes(header_region))
    actual_header_sig = _read_exact(fh, HEADER_DIGEST_OFFSET, SIG_SIZE)
    if actual_header_sig != expected_header_sig:
        errors.append("header signature region digest mismatch")

    for inode in inodes:
        remaining = inode.blocks
        direct_count = min(remaining, MAX_DIRECT_BLOCKS)
        ...
        sigs_per_block = header.block_size // SIG_ENTRY_SIZE
        if remaining > 0:
            ib0 = inode.ib[0]
            if ib0 <= 0:
            ...
            if ib1 <= 0:
                ...
                for parent_idx, (parent_sig, child_indirect) in enumerate(parent_records):
                    ...
                    if parent_sig != hmac_sha256(sign_key, child_data):
                        errors.append(
                            f"inode {inode.number} ib[1] record {parent_idx} signature mismatch for block {child_indirect}"
                        )
                    ...
                    for rec_idx, (sig, block) in enumerate(child_records[:take]):
                        ...
                        if sig != expected:
                            errors.append(
                                f"inode {inode.number} ib[1][{parent_idx}] record {rec_idx} signature mismatch for block {block}"
                            )
                    remaining -= take
...
def resolve_signed_inode_blocks(
    fh, header: ParsedHeader, inode: ParsedInode, errors: list[str] | None = None
) -> list[int]:
    blocks: list[int] = []
    direct_count = min(inode.blocks, MAX_DIRECT_BLOCKS)
    blocks.extend(inode.db[:direct_count])
...
def read_image_inode_payload(fh, inode: ParsedInode, block_size: int) -> bytes:
    if inode.blocks <= 0:
        return b""
...
def build_tree_from_uroot(
    ...
    def walk(dir_inode_num: int, rel_path: str, parent_inode_num: int, ancestors: set[int]) -> None:
        ...
        elif dot.type_code != DIRENT_TYPE_DOT:
        ...
        if dotdot is None:
            ...
            if dotdot.type_code != DIRENT_TYPE_DOTDOT:
        ...
        for ent in entries:
            ...
            if ent.type_code == DIRENT_TYPE_DIRECTORY:
                ...
                if prev_path is not None and prev_path != child_path:
                    errors.append(
                        f"directory inode {ent.inode_number} is reachable from multiple paths: '{prev_path}' and '{child_path}'"
                    )
                    continue
            ...
            elif ent.type_code == DIRENT_TYPE_FILE:
...
def render_tree(dirents_by_inode: dict[int, list[ParsedDirent]], inode_num: int, prefix: str = "") -> list[str]:
    ...
    entries.sort(key=lambda e: (e.type_code != DIRENT_TYPE_DIRECTORY, e.name.lower(), e.name))
...
def validate_inode_layout(
    ...
    if header.magic != PFS_MAGIC:
        errors.append(f"header magic mismatch: 0x{header.magic:016X} != 0x{PFS_MAGIC:016X}")
    ...
    for i in range(1, len(used_ranges)):
        ...
        if curr_start <= prev_end:
            errors.append(
                f"block overlap between inode {prev_ino} [{prev_start},{prev_end}] and inode {curr_ino} [{curr_start},{curr_end}]"
            )
    ...
    for h in sorted(expected_hashes & table_hashes):
        ...
        if len(exp_entries) == 1:
            ...
            if (val & 0x80000000) == 0:
                ...
                continue
            actual_set = {
                (e.name, e.type_code == DIRENT_TYPE_DIRECTORY, e.inode_number) for e in collision_map.get(h, [])
            }
...
def run_image_check(
    ...
    with image.open("rb") as fh:
        ...
        if uroot_inode >= 0:
            ...
            case_insensitive = bool(header.mode & PFS_MODE_CASE_INSENSITIVE)
            ...
            if emit_report:
                ...
                print("=" * 70)
                print(f"Image:                 {image}")
                _ver_label = "PS5" if header.version == PFS_VERSION_PS5 else "PS4"
                print(f"Version:               {header.version} ({_ver_label})")
                print(f"Format:                {header.magic}")
                print(f"Read-only:             {'yes' if header.readonly else 'no'}")
                print(
                    f"Mode:                  0x{header.mode:04X}  (Bit 0=signed, Bit 1=64-bit inodes, Bit 2=encrypted, Bit 3=case insensitive)"
                )
                print(f"  Signed:              {'yes' if header.mode & PFS_MODE_SIGNED else 'no'}")
                print(f"  64-bit inodes:       {'yes' if header.mode & PFS_MODE_64BIT_INODES else 'no'}")
                print(f"  Encrypted:           {'yes' if header.mode & PFS_MODE_ENCRYPTED else 'no'}")
                print(f"  Case insensitive:    {'yes' if header.mode & PFS_MODE_CASE_INSENSITIVE else 'no'}")
